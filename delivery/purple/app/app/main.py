...
st.set_page_config(page_title="ACV | Train condition monitoring",page_icon="🚆",layout="wide")
st.markdown("""<style>
...
REGISTRY={"ACV · Air conditioning": {"artifact":ROOT/"artifacts/acv/final","extensions":["xlsx"]}}
...
with st.sidebar:
    st.header("Analysis")
    subsystem=st.selectbox("Subsystem",list(REGISTRY))
    st.caption("ACV is available. Additional subsystems can be registered as their models are validated.")
    st.divider()
    st.write("**How to use**")
    st.write("1. Upload the telemetry workbook.\n2. Run the analysis.\n3. Review and download the results.")
    st.caption("The file is processed locally by this app.")

artifact=REGISTRY[subsystem]["artifact"]
...
@st.cache_resource
def model(path,stamp):return load_artifact(path)

bundle=model(str(artifact),(artifact/"manifest.json").stat().st_mtime_ns)
manifest=json.loads((artifact/"manifest.json").read_text())
uploads=st.file_uploader("Upload ACV telemetry",type=["xlsx"],accept_multiple_files=True,help="Each workbook should contain one telemetry sheet with Time and Car <ID> columns.")
...
if run or demo_clicked:
    ...
    if len({n for n,_ in sources})!=len(sources):st.error("Choose files with distinct filenames.");st.stop()
    results=[]
    ...
    try:
        ...
        st.session_state.stabilities={}
        # An explicit Analyze action is the source of submission exports.
        export=ROOT/"outputs/acv/app_export";export.mkdir(parents=True,exist_ok=True)
        ...
        save_json(export/"provenance.json",{"source":"streamlit_app","input_hashes":{r.file_id:r.features.case.metadata["source_hash"] for r in results},"model_sha256":manifest["model_sha256"]})
...
for warning in r.warnings:st.caption(warning)
...
with ranking_tab:
    ...
    fig.update_yaxes(type="category",categoryorder="array",categoryarray=r.ranked_cars[::-1]);fig.update_layout(showlegend=False,coloraxis_showscale=False,height=350,margin=dict(t=10,b=20))
    st.plotly_chart(fig,width="stretch")
    view=r.table[["rank","score","evidence","usable_hours","err_mean","peer_mean"]].rename(columns={"rank":"Rank","score":"Relative evidence","evidence":"Data status","usable_hours":"Cooling hours","err_mean":"Mean setpoint error (°C)","peer_mean":"Mean peer difference (°C)"})
    ...
    if st.button("Check ranking stability"):
        with st.spinner("Resampling synchronized one-hour blocks…"):
            st.session_state.stabilities[r.file_id]=bootstrap_stability(r.features,bundle)
    stable=st.session_state.stabilities.get(r.file_id)
    if stable:
...
with evidence_tab:
    ...
    st.plotly_chart(px.line(plot,x="time",y="cabin",color="car",labels={"time":"Time","cabin":"Cabin temperature (°C)","car":"Car"}),width="stretch")
    st.plotly_chart(px.line(plot,x="time",y="peer",color="car",labels={"time":"Time","peer":"Setpoint error above peers (°C)","car":"Car"}),width="stretch")
...
with quality_tab:
    ...
    st.write("Missing sensors, invalid observations, and unavailable cars are tracked separately. Unavailable telemetry does not establish that a car is healthy.")
