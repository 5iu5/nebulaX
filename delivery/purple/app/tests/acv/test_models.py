...
import pandas as pd
import pytest
from acv.models import Model, candidates
...
@pytest.mark.parametrize("rank,score",[(1,1.),(2,.875),(8,.125)])
...
def test_missing_car_neutral_not_dropped():
    s=percentiles(pd.Series({"01":3.,"02":1.,"03":np.nan}))
    assert s["03"]==.5 and ordered(s)==["01","03","02"]
...
def test_baseline_and_submission(features,baseline):
    ...
    with pytest.raises(ValueError,match="Duplicate input"):predictions_csv([r,r])

@pytest.mark.parametrize("name,params",[("linear_ranker",{"C":.1}),("healthy_residual",{"alpha":1.,"aggregation":"mean"})])
...
def test_evaluator_rejects_fold_overlap(features,tmp_path):
    ...
    with pytest.raises(AssertionError,match="leakage"):ev.predict("thermal",{"aggregation":"mean"},["a"],["a"])

def test_bootstrap_reproducible(features,baseline):
    from acv.robustness import bootstrap_stability
    a=bootstrap_stability(features,baseline,samples=20);b=bootstrap_stability(features,baseline,samples=20)
    assert a==b and a["cars"]["03"]["top1_frequency"]==1.

def test_neural_shared_encoder_equivariance():
    torch=pytest.importorskip("torch")
    from acv.neural import TemporalMIL
    torch.manual_seed(42);m=TemporalMIL().eval()
    x=torch.randn(4,2,9,120);mask=torch.ones(4,2,120);perm=[2,0,3,1]
    with torch.no_grad():a=m(x,mask);b=m(x[perm],mask[perm])
...
def test_artifact_checksum_refuses_tampering(tmp_path):
    ...
    with pytest.raises(ValueError,match="checksum"):load_artifact(tmp_path)
