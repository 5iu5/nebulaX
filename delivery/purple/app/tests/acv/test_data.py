...
def test_shuffled_columns_and_ids(wide):
    a=extract_features(from_wide(wide));b=extract_features(from_wide(wide.sample(frac=1,axis=1,random_state=9)))
...
def test_renaming_cars_is_equivariant(wide):
    ...
    for c in a.index:assert a[c]==pytest.approx(b[mapping[c]])
...
def test_quality_masks_are_distinct(wide):
    ...
    c=from_wide(wide);f=c.native.loc[c.native.car.eq("01")]
...
def test_fill_only_bounded_short_same_state():
    s=pd.Series([1.,np.nan,np.nan,4.,np.nan,np.nan,np.nan,8.])
    state=pd.DataFrame({"mode":["cool"]*8,"target":[24]*8})
    out=_fill_short(s,state)
    assert out.iloc[1:3].tolist()==[2.,3.] and out.iloc[4:7].isna().all()
...
def test_unavailable_car_is_retained(wide):
    for col in wide:
        if col.startswith("Car 08 - "):wide[col]="None"
...
def test_rich_operating_invalid_does_not_discard(wide):
    wide=wide.rename(columns=lambda c:c.replace("Indoor Average Temperature","Passenger Cabin Temperature Detected Value").replace("ACV Control Temperature (Cooling)","Target Temperature Value"))
...
def test_reject_bad_time_and_duplicate_headers(wide):
    bad=wide.copy();bad.loc[1,"Time"]=bad.loc[0,"Time"]
    with pytest.raises(ValueError,match="Duplicate timestamps"):from_wide(bad)
    with pytest.raises(ValueError,match="Time column"):from_wide(wide.drop(columns="Time"))
...
def test_missing_ambient_spike_and_short_gap_keep_complete_ranking(wide, baseline):
    ...
    for col in [c for c in wide if c.endswith("Outdoor Average Temperature")]:
        wide[col]=np.nan
    wide.loc[40,"Car 03 - Indoor Average Temperature"]=500.
