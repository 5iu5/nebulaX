...
def test_six_workbooks_and_frozen_baseline():
    ...
    for n,f in data.items():
        r=ordered(percentiles(thermal_scores(f),f.table.available));ranks.append(r.index(labels[n])+1)
        assert len(r)==8
...
def test_app_upload_service_cli_parity(tmp_path):
    ...
    if not artifact.exists():pytest.skip("Final artifact not trained yet")
    p=ROOT/"PS3/02_Datasets/ACV/Test/acv_test_case.xlsx"
    bundle=load_artifact(artifact)
    a=rank_case(p,bundle);b=rank_case(io.BytesIO(p.read_bytes()),bundle,file_id=p.name)
    assert predictions_csv([a])==predictions_csv([b])
    output=tmp_path/"acv_predictions.csv";output.write_bytes(predictions_csv([a]))
