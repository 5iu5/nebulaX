...
def load_artifact(path):
    ...
    if digest(path/"model.joblib")!=manifest["model_sha256"]:raise ValueError("Model artifact checksum mismatch")
...
def rank_case(source,artifact,file_id=None):
    ...
    if bundle.get("challenger") is not None:
        other=percentiles(bundle["challenger"].predict(f),f.table.available);w=bundle["weight"]
        scores=(1-w)*scores+w*other
    cars=ordered(scores);table=f.table.copy();table["score"]=scores
    table["evidence"]=table.available.map({True:"Available",False:"Unavailable"})
    table=table.loc[cars];table.insert(0,"rank",range(1,len(cars)+1))
    warnings=[]
    missing=table.index[~table.available].tolist()
    if missing:warnings.append("No usable cooling evidence for cars "+", ".join(missing)+". They retain a neutral score, not a healthy diagnosis.")
    if table.usable_hours.max()<1:warnings.append("Less than one hour of usable cooling data; ranking stability is limited.")
...
def predictions_csv(results):
    ...
    if len(names)!=len(set(names)):raise ValueError("Duplicate input filenames would create ambiguous submission rows")
    rows=[]
    for r in results:
...
def write_predictions(results,path):
    path=Path(path);path.parent.mkdir(parents=True,exist_ok=True);path.write_bytes(predictions_csv(results));return path

def prediction_zip(csv_bytes):
    buf=io.BytesIO()
    with zipfile.ZipFile(buf,"w",zipfile.ZIP_DEFLATED) as z:
        info=zipfile.ZipInfo("acv_predictions.csv",date_time=(2026,1,1,0,0,0));info.compress_type=zipfile.ZIP_DEFLATED
