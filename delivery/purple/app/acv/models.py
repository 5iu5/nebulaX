...
def candidates(name):
    if name == "thermal": return [{"aggregation": a} for a in AGGREGATIONS]
    if name == "healthy_residual": return [{"alpha": x, "aggregation": a} for x,a in itertools.product([.1,1.,10.], AGGREGATIONS)]
    if name == "linear_ranker": return [{"C": x} for x in [.01,.1,1.]]
    if name == "catboost_ranker": return [dict(depth=d,iterations=i,l2_leaf_reg=l) for d,i,l in itertools.product([2,3],[100,300],[10,30])]
    if name == "tcn_mil": return [{"max_epochs":100}]
...
class Model:
    ...
    def fit(self, data, labels, device="auto", deadline=None):
        ...
        if self.name == "thermal": return self
        if self.name == "healthy_residual":
            ...
            for name, feat in sorted(data.items()):
                ...
                x = behaviour_frame(feat).loc[mask]; y = f.loc[mask,"err"]
                if not len(x): continue
                inputs.append(x); targets.append(y); weights.extend([1/len(x)]*len(x))
            if not inputs: raise ValueError("No valid healthy-car training observations")
            x = pd.concat(inputs); y = pd.concat(targets)
            ...
            w = np.asarray(weights); w *= len(w)/w.sum()
        ...
        if self.params.get("quality"): self.feature_names += QUALITY_FEATURES[:max(0,32-len(self.feature_names))]
        tables = []; groups = []; ys = []
        for number,(name,feat) in enumerate(sorted(data.items())):
            x = feat.table.loc[feat.table.available,self.feature_names]
            tables.append(x); groups.extend([number]*len(x)); ys.extend([int(c==labels[name]) for c in x.index])
        x = pd.concat(tables)
        if self.name == "linear_ranker":
            ...
            for group in sorted(set(groups)):
                ...
                pos = idx[np.asarray(ys)[idx]==1]; neg=idx[np.asarray(ys)[idx]==0]
                if not len(pos) or not len(neg): continue
                for n in neg:
                    diff = xx[pos[0]]-xx[n]
                    pairs.extend([diff,-diff]); target.extend([1,0]); weight.extend([1/(2*len(neg))]*2)
            if not pairs: raise ValueError("No positive/negative car pairs available")
            ...
            for g in sorted(sizes):
                ...
                pos=idx[np.asarray(ys)[idx]==1]; neg=idx[np.asarray(ys)[idx]==0]
                if len(pos):
                    for n in neg: pairs.append((int(pos[0]),int(n))); pw.append(1/max(1,len(neg)))
            pool=Pool(x,label=ys,group_id=groups,pairs=pairs,pairs_weight=pw)
            self.estimator=[]
            for seed in [17,42,2026]:
                model=CatBoostRanker(loss_function="PairLogit",learning_rate=.03,random_seed=seed,
                      verbose=False,allow_writing_files=False,thread_count=2,
                      **{k:v for k,v in self.params.items() if k!="quality"})
                model.fit(pool); self.estimator.append(model)
        return self

    def predict(self, features):
        if self.name == "thermal": return thermal_scores(features,self.params["aggregation"])
        if self.name == "healthy_residual":
            f=features.case.frame; pred=self.estimator.predict(self.scaler.transform(behaviour_frame(features)))
        ...
        if self.name == "linear_ranker": values=self.estimator.decision_function(self.scaler.transform(x))
        else: values=np.mean([model.predict(x) for model in self.estimator],axis=0)
