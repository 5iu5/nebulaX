...
def _fill_short(series, states, limit=2):
    ...
    for start, end in zip(np.where(edges == 1)[0], np.where(edges == -1)[0]):
        if start == 0 or end == len(s) or end-start > limit: continue
        block = states.iloc[start-1:end+1]
        if block.isna().any().any() or not block.eq(block.iloc[0]).all().all(): continue
...
def _resample_modes(frame):
    ...
    out=pd.DataFrame(result);out.index=pd.DatetimeIndex(out.index,name="time")
    return out

def from_wide(df, file_id="case.xlsx", source_hash="memory"):
    if "Time" not in df: raise ValueError("Workbook must contain a Time column.")
    if df.columns.duplicated().any(): raise ValueError("Duplicate column headers are ambiguous.")
    mapping = {}
    for col in df:
        m = HEADER.match(str(col))
        if m: mapping.setdefault(m[1], {})[m[2]] = col
    if not mapping: raise ValueError("No 'Car <ID> - <parameter>' columns found.")
    time = pd.to_datetime(df.Time, errors="coerce")
    if time.isna().any(): raise ValueError("Time contains missing or unparseable timestamps.")
    if time.duplicated().any(): raise ValueError("Duplicate timestamps require source correction.")
    order = np.argsort(time.to_numpy(), kind="stable")
    df = df.iloc[order].reset_index(drop=True); time = pd.DatetimeIndex(time.iloc[order], name="time")
    native = []
    for car, columns in sorted(mapping.items()):
        ...
        for key, names in ALIASES.items():
            ...
            if len(found) > 1: raise ValueError(f"Ambiguous aliases for car {car}: {key}")
            ...
            if key in NUMERIC:
                ...
                if key in {"cabin", "observation"}: val = val.where(val.between(5, 50))
                if key == "target": val = val.where(val.between(10, 35))
                if key == "ambient": val = val.where(val.between(-10, 65))
                if key.startswith("p"): val = val.where(val > 0)
    ...
    for car, f in native.groupby("car", sort=True):
        ...
        for c in NUMERIC:
            ...
            if c == "target" or c.startswith("compressor"): continue
    ...
    meta = {"source_hash": source_hash, "rows": len(df), "columns": len(df.columns),
            ...
            "start": str(time.min()), "end": str(time.max()),
...
def enrich(case):
    for attr in ("native", "frame"):
        ...
        for car in case.cars:
            loc = f.car.eq(car); own = f.loc[loc]
...
def load_case(source, file_id=None):
    if isinstance(source, (str, Path)):
        path = Path(source); content = path.read_bytes(); name = file_id or path.name
    else:
    ...
    for sheet in book.sheet_names:
        ...
        if "Time" in headers and any(HEADER.match(str(c)) for c in headers): matches.append(sheet)
    if len(matches) != 1: raise ValueError("Expected exactly one telemetry worksheet with Time and car headers.")
