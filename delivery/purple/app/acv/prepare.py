...
def training_data(cfg):
    ...
    if set(data) != set(labels): raise ValueError("Training files and labels do not match.")
    for name, car in labels.items():
        if car not in data[name].case.cars: raise ValueError(f"Unknown faulty car {car} in {name}")
    return data, labels

def main():
    p = argparse.ArgumentParser(); p.add_argument("--config", default="configs/acv.yaml")
    cfg = config(p.parse_args().config); data, labels = training_data(cfg)
