...
LINEAR_FEATURES = ["peer_mean", "peer_p90", "peer_p95", "err_mean", "err_p90", "positive_error", "peer_gt_05", "peer_gt_1", "peer_long_1", "slope_5", "manual_fraction", "mode_disagreement"]
TREE_FEATURES = LINEAR_FEATURES + ["err_median", "err_p95", "peer_gt_2", "peer_long_05", "peer_long_2", "slope_15", "startup_error", "steady_error", "cooling_fraction", "full_fraction", "half_fraction", "late_change", "daily_max", "block15_p90", "block60_p90", "block180_p90", "ambient_error", "block60_mean", "recovery_15", "positive_peer"]
...
def aggregate(series, kind="mean"):
    ...
    if kind == "mean": return float(valid.mean())
    counts = series.resample("60min").count()
    means = series.resample("60min").mean().where(counts >= 84).dropna()
    if means.empty: return float(valid.mean())
    if kind == "block_mean": return float(means.mean())
    if kind == "block_tail": return float(0.5*means.mean()+0.5*means.quantile(.9))
...
def extract_features(case):
    ...
    for car in case.cars:
        ...
        e, p = f.err, f.peer
        usable = f.eligible
        row = {"car": car, "available": bool(usable.any()), "usable_hours": float(usable.sum()/120),
               ...
               "err_mean": e.mean(), "err_median": e.median(), "err_p90": e.quantile(.9), "err_p95": e.quantile(.95),
               "peer_mean": p.mean(), "peer_p90": p.quantile(.9), "peer_p95": p.quantile(.95),
               "positive_error": e.clip(lower=0).mean(), "positive_peer": p.clip(lower=0).mean(),
               "slope_5": f.slope_5.mean(), "slope_15": f.slope_15.mean(),
               "startup_error": e.where(f.since_transition.lt(300)).mean(),
               "steady_error": e.where(f.steady).mean(),
               "cooling_fraction": usable.mean(), "full_fraction": f["mode"].eq("Full Cooling").mean(),
               "half_fraction": f["mode"].eq("Half Cooling").mean(),
               "manual_fraction": f.control.eq("Manual Control").where(f.control.notna()).mean(),
               "invalid_events": f.invalid_events.sum(), "mode_events": f.mode_events.sum(),
...
def thermal_scores(features, aggregation="mean"):
    if aggregation == "mean": return features.table.native_thermal.copy()
...
def percentiles(scores, available=None):
    ...
    if available is not None: scores = scores.where(pd.Series(available).reindex(scores.index).fillna(False))
    good = scores.dropna()
    if len(good) <= 1:
        out = pd.Series(.5, index=scores.index)
        if len(good): out.loc[good.index] = 1.
        return out
    return ((scores.rank(method="average")-1)/(len(good)-1)).fillna(.5)
