...
class ResidualBlock(nn.Module):
    def __init__(self, dilation):
        super().__init__()
        self.net=nn.Sequential(nn.Conv1d(32,32,3,padding=dilation,dilation=dilation),nn.ReLU(),nn.Dropout(.2),
                               nn.Conv1d(32,32,3,padding=dilation,dilation=dilation),nn.ReLU(),nn.Dropout(.2))
    def forward(self,x): return x+self.net(x)

class TemporalMIL(nn.Module):
    def __init__(self, channels=len(CHANNELS)):
        super().__init__()
...
def windows(features):
    case=features.case; timeline=case.frame.index.unique().sort_values()
    start=timeline.min().floor("h"); end=timeline.max().ceil("h")
    grid=pd.date_range(start,end,freq="30s",inclusive="left")
    if len(grid)<120: grid=pd.date_range(start,periods=120,freq="30s")
    all_x,all_m=[],[]
    for car in case.cars:
        ...
        x=pd.DataFrame({"err":f.err,"peer":f.peer,"cabin_ambient":f.cabin-f.ambient_shared,"delta":delta,
             "automatic":f["mode"].eq("Automatic Cooling").astype(float),
             "full":f["mode"].eq("Full Cooling").astype(float),"half":f["mode"].eq("Half Cooling").astype(float),
             "observed":f.observed.fillna(False).astype(float),"eligible":f.eligible.fillna(False).astype(float)})
        a=x.to_numpy(dtype=np.float32); m=f.eligible.fillna(False).to_numpy(dtype=np.float32)
        pad=(-len(a))%120
        if pad: a=np.pad(a,((0,pad),(0,0)),constant_values=np.nan); m=np.pad(m,(0,pad))
        a=a.reshape(-1,120,len(CHANNELS)).transpose(0,2,1);m=m.reshape(-1,120)
        m[m.mean(1)<.7]=0
        all_x.append(a);all_m.append(m)
    xx=np.stack(all_x);mm=np.stack(all_m)
...
def train_neural(data,labels,params,device="auto",deadline=None):
    device=choose_device(device);torch.set_num_threads(2)
    ...
    for x,m in batches.values():
        ...
        if len(flat): samples.append(flat[::max(1,len(flat)//2000)])
    if not samples: raise ValueError("No eligible neural windows")
    joined=np.concatenate(samples)
    median=np.nanmedian(joined,axis=0);scale=np.nanpercentile(joined,75,axis=0)-np.nanpercentile(joined,25,axis=0)
    median=np.nan_to_num(median);scale=np.where(np.isfinite(scale)&(scale>.01),scale,1.)
    median[4:]=0;scale[4:]=1
    tensors={n:(torch.tensor(np.nan_to_num((x-median[None,None,:,None])/scale[None,None,:,None]),device=device),
                torch.tensor(m,device=device)) for n,(x,m) in batches.items()}
    targets={n:data[n].case.cars.index(labels[n]) for n in data}
    states=[];epoch_choices=[]
    ...
    def fit_pass(names,seed,epochs,validation=None):
        ...
        model=TemporalMIL().to(device);optim=torch.optim.AdamW(model.parameters(),lr=1e-3,weight_decay=1e-3)
        best_loss=float("inf");best_epoch=1;stale=0
        for epoch in range(epochs):
            if deadline and time.monotonic()>deadline: raise TimeoutError("Neural experiment time budget exhausted")
            ...
            for name in names:
                ...
                loss.backward()
            nn.utils.clip_grad_norm_(model.parameters(),1.);optim.step()
            if validation:
                model.eval()
                with torch.no_grad():
                    x,m=tensors[validation];idx=np.linspace(0,x.shape[1]-1,min(16,x.shape[1]),dtype=int)
                    score=model(x[:,idx],m[:,idx]);vl=float(nn.functional.cross_entropy(score.unsqueeze(0),torch.tensor([targets[validation]],device=device)))
                if vl<best_loss-1e-4: best_loss=vl;best_epoch=epoch+1;stale=0
                else: stale+=1
                if stale>=10: break
    ...
    for seed in seeds:
        ...
        else: epochs=min(20,max_epochs)
        model,_=fit_pass(names,seed,epochs)
        states.append({k:v.detach().cpu() for k,v in model.state_dict().items()});epoch_choices.append(epochs)
    return {"states":states,"median":median,"scale":scale,"channels":CHANNELS,"epochs":epoch_choices,"device_trained":device,
            "early_stop_training_files":names[:-1],"early_stop_validation_file":names[-1] if len(names)>2 else None}
...
def predict_neural(bundle,features):
    ...
    xt=torch.tensor(x[:,idx],dtype=torch.float32);mt=torch.tensor(m[:,idx],dtype=torch.float32)
    result=[]
    with torch.no_grad():
        for state in bundle["states"]:
            model=TemporalMIL();model.load_state_dict(state);model.eval();result.append(model(xt,mt).numpy())
