...
def main():
    ...
    p.add_argument("--team", default="purple")
    p.add_argument("--stage-only", action="store_true", help="Stage files but explicitly mark a missing demo as incomplete")
    a = p.parse_args()
    if Path(a.team).name != a.team or a.team in [".", ".."]:
        raise ValueError("Team name must be a single folder name")
    export = ROOT / "outputs/acv/app_export"
    provenance = json.loads((export / "provenance.json").read_text())
    if provenance["source"] != "streamlit_app":
        raise ValueError("Generate final predictions through the app")
    ...
    if set(provenance["input_hashes"]) != expected:
        raise ValueError("Input provenance must cover every test file")
    for name, sha in provenance["input_hashes"].items():
        if digest(ROOT / "PS3/02_Datasets/ACV/Test" / name) != sha:
    ...
    if video.exists():
        if not shutil.which("ffprobe"):
            raise ValueError("Install ffmpeg/ffprobe to verify the demo duration")
        duration = float(
        ...
        if not 0 < duration <= 180:
            raise ValueError("The demo must be nonempty and no longer than three minutes")
    elif not a.stage_only:
        raise ValueError("Record the app demo before packaging the final submission")
    team = ROOT / "delivery" / a.team
    app = team / "app"
    app.mkdir(parents=True, exist_ok=True)
    for name in ["acv", "door", "shm", "app", "configs", ".streamlit", "artifacts/acv/final", "scripts", "docs", "tests"]:
        shutil.copytree(ROOT / name, app / name, dirs_exist_ok=True, ignore=shutil.ignore_patterns("__pycache__"))
    for name in ["pyproject.toml", "requirements.lock", "requirements-catboost.txt", "README_ACV.md"]:
        shutil.copy2(ROOT / name, app / name)
    shutil.copy2(ROOT / "predict.py", app / "predict.py")
    ...
    validation = optional / "ACV" / "validation"
    ...
    for name in ["provenance.json", "verification.json"]:
        if (export / name).exists():
            shutil.copy2(export / name, validation / name)
    if (ROOT / "outputs/acv/tests.xml").exists():
        shutil.copy2(ROOT / "outputs/acv/tests.xml", validation / "tests.xml")
    if video.exists():
        shutil.copy2(video, team / "demo_video.mp4")
    (team / "DELIVERY_STATUS.json").write_text(
        json.dumps(
            {"complete": video.exists(), "missing": [] if video.exists() else ["Required live app demo video"], "team": a.team, "demo_seconds": duration},
            indent=2,
        )
    )
    (team / "START_HERE.txt").write_text(
        "ACV submission for "
        + a.team
