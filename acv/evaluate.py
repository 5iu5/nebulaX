...
class Evaluator:
    def __init__(self, data, labels, cfg, device="auto", deadline=None):
        ...
        self.cache = Path(cfg["output_root"]) / "fit_cache"
    ...
    def choose(self, name, names, grid=None):
        ...
        for params in candidates("thermal"):
            pred = self.cv("thermal", params, names)
            rows = self.rows(pred)
            baseline.append((selection_key(rows), params, pred, rows))
        _, base_params, base_pred, base_rows = max(baseline, key=lambda a: a[0])
        best = {"model": "thermal", "params": base_params, "baseline_params": base_params, "weight": 0.0, "inner_summary": summary(base_rows)}
        ...
        for params in grid or candidates(name):
            ...
            for weight in [0.25, 0.5, 0.75, 1.0]:
                ...
                rows = self.rows(blended)
                metrics = summary(rows)
                # Strict benefit over baseline; no worst-case regression.
                if metrics["primary_metric"] <= best_baseline(base_rows) or metrics["worst_rank"] > summary(base_rows)["worst_rank"]:
                    continue
                candidate_key = selection_key(rows, MODEL_NAMES.index(name))
                if candidate_key > key:
                    key = candidate_key
                    best = {"model": name, "params": params, "baseline_params": base_params, "weight": weight, "inner_summary": metrics}
        return best

    def apply(self, selection, train, test):
        base = self.predict("thermal", selection["baseline_params"], train, test)
        if selection["model"] == "thermal":
            return base
        pred = self.predict(selection["model"], selection["params"], train, test)
        w = selection["weight"]
        return {n: (1 - w) * base[n] + w * pred[n] for n in test}


def best_baseline(rows):
    return summary(rows)["primary_metric"]
...
def evaluate(name, cfg, device="auto", max_epochs=100, quick=False):
    ...
    if name == "tcn_mil":
        grid = [{"max_epochs": max_epochs, **({"seeds": [42]} if quick else {})}]
    if quick and name != "tcn_mil":
        grid = grid[:1]
    root = Path(cfg["output_root"]) / "evaluation"
    ...
    report = {
        ...
        "quick": quick,
        "candidate_grid": grid,
        "case_hashes": {n: data[n].case.metadata["source_hash"] for n in names},
        "outer_folds": [],
        "interpretation": "Development validation on six cases; initial exploratory analysis used all six. Not hidden-test accuracy.",
    }
    ...
    try:
        for test in names:
            ...
            report["outer_folds"].append(
                {**result, "selection": choice, "training_files": train, "training_hashes": {n: data[n].case.metadata["source_hash"] for n in train}}
            )
        ...
        standalone = []
        for params in grid:
            rows = ev.rows(ev.cv(name, params, names))
            standalone.append({"params": params, "summary": summary(rows), "cases": rows})
