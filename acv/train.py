...
def train(selection, cfg, device="auto"):
    ...
    if actual != selection["case_hashes"]:
        raise ValueError("Training inputs differ from evaluated data")
    if selection["code_hash"] != code_hash():
        raise ValueError("Code changed after evaluation; reevaluate before final training")
    base = Model("thermal", selection["baseline_params"]).fit(data, labels)
    challenger = None
    if selection["model"] != "thermal":
        challenger = Model(selection["model"], selection["params"]).fit(data, labels, device)
    bundle = {"baseline": base, "challenger": challenger, "weight": selection["weight"], "selection": selection}
    root = Path(cfg["artifact_root"]) / "final"
    root.mkdir(parents=True, exist_ok=True)
    joblib.dump(bundle, root / "model.joblib", compress=3)
    manifest = {
        "format_version": 1,
        "code_hash": code_hash(),
        "model_sha256": digest(root / "model.joblib"),
        "versions": versions(),
        "training_files": sorted(data),
        "training_hashes": actual,
        "selection": selection,
...
def main():
    ...
    a = p.parse_args()
    train(json.loads(Path(a.selection).read_text()), config(a.config), a.device)
