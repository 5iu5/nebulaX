...
def select(results, cfg):
    ...
    for path in sorted(Path(results).glob("*.json")):
        ...
        if r.get("status") != "complete" or r.get("quick"):
            ...
            continue
        reports.append(r)
    if not any(r["model"] == "thermal" for r in reports):
        raise ValueError("Complete thermal nested evaluation is required")
    ...
    selection = {
        **final,
        "validation_summary": summary(outer),
        "nested_selector_cases": outer,
        "baseline_summary": baseline["summary"],
        "available_models": [r["model"] for r in reports],
        "excluded_models": excluded,
        "case_hashes": baseline["case_hashes"],
...
def main():
    ...
    p.add_argument("--results", default="outputs/acv/evaluation")
    p.add_argument("--config", default="configs/acv.yaml")
    a = p.parse_args()
    select(a.results, config(a.config))
